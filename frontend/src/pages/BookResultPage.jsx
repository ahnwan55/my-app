...
function parseAiComment(text) {
  ...
  const parseBold = (str) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith("**") && part.endsWith("**")) {
        return <strong key={i} style={{ fontWeight: 700, color: C.gray800 }}>{part.slice(2, -2)}</strong>;
      }
      return part;
    });
  };
    ...
    elements.push(
        <ul key={key++} style={{ margin: "6px 0", paddingLeft: 18, display: "flex", flexDirection: "column", gap: 4 }}>
          {listBuffer.map((item, i) => (
              <li key={i} style={{ fontSize: 13, color: C.gray700, lineHeight: 1.65 }}>{parseBold(item)}</li>
          ))}
        </ul>
    );
    listBuffer = [];
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith("> ")) {
      flushList();
      elements.push(
          <div key={key++} style={{ borderLeft: `3px solid ${C.pink}`, paddingLeft: 10, margin: "4px 0", color: C.gray500, fontSize: 12, lineHeight: 1.65, fontStyle: "italic" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {isLong && (
                <button
                    ...
                    style={{ background: "none", border: "none", color: C.pink, fontSize: 12, fontWeight: 700, cursor: "pointer", padding: "4px 0 0", fontFamily: "'Noto Sans KR', sans-serif" }}
                >
                  {expanded ? "접기 ▲" : "더 보기 ▼"}
                </button>
            )}
...
export default function BookResultPage({ personaName: propPersonaName = "지적 탐험가" }) {
  ...
  return (
      <div style={S.wrap}>
        ...
        <div style={S.header}>
          {/* ── 홈으로 버튼 ── */}
          <button onClick={() => navigate("/main")} style={S.homeBtn}>← 홈으로</button>
          ...
          {data?.reason && !data?.aiComment && <p style={S.reason}>{data.reason}</p>}
        ...
        {selected && (
            <div style={S.overlay} onClick={() => setSelected(null)}>
              <div style={S.modal} onClick={e => e.stopPropagation()}>
                ...
                <button style={S.detailBtn} onClick={() => navigate(`/books/${selected.book.bookId}`)}>
                  📖 상세 보기
                </button>
              </div>
            </div>
  ...
  return (
      <div
          style={{
            ...S.card,
            opacity:   visible ? 1 : 0,
            transform: visible ? hover ? "translateX(4px)" : "translateX(0)" : "translateY(16px)",
            transition: `opacity 0.5s ease ${delay}ms, transform 0.25s ease`,
            boxShadow: hover ? "0 8px 24px rgba(244,114,182,0.2)" : "0 2px 10px rgba(244,114,182,0.08)",
            cursor: "pointer",
          }}
          onClick={onClick}
      ...
      >
        ...
        <div style={S.coverBox}>
          {book.coverUrl ? <img src={book.coverUrl} alt={book.title} style={S.coverImg} /> : <BookIcon size={26} />}
        </div>
...
const S = {
  ...
  homeBtn:     { fontSize: 13, color: C.gray400, background: "none", border: "none", cursor: "pointer", padding: "0 0 12px", fontFamily: "'Noto Sans KR', sans-serif", display: "block" },
  headerSub:   { fontSize: 13, color: C.purple, fontWeight: 700, margin: "0 0 6px", letterSpacing: "0.05em" },
  headerTitle: { fontSize: 24, fontWeight: 900, margin: "0 0 12px", background: `linear-gradient(135deg, ${C.pink}, ${C.purple})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" },
  aiComment: { background: "rgba(255,255,255,0.85)", border: `1.5px solid ${C.pinkLight}`, borderRadius: 16, padding: "14px 16px", margin: "0 0 10px", backdropFilter: "blur(8px)", textAlign: "left" },
  ...
  list: { position: "relative", zIndex: 1, maxWidth: 480, margin: "0 auto", padding: "0 16px", display: "flex", flexDirection: "column", gap: 10 },
  card: { background: "rgba(255,255,255,0.85)", backdropFilter: "blur(12px)", border: `1.5px solid ${C.pinkLight}`, borderRadius: 18, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14, position: "relative" },
  badge: { flexShrink: 0, width: 24, height: 24, borderRadius: "50%", background: `linear-gradient(135deg, ${C.pink}, ${C.purple})`, color: C.white, fontSize: 11, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center" },
  coverBox: { flexShrink: 0, width: 52, height: 68, borderRadius: 8, background: C.pinkLight, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" },
  coverImg:  { width: "100%", height: "100%", objectFit: "cover" },
  cardBody:  { flex: 1, minWidth: 0 },
  genreTag:  { fontSize: 9, fontWeight: 800, color: C.purple, letterSpacing: "0.1em", textTransform: "uppercase" },
  cardTitle: { fontSize: 14, fontWeight: 800, color: C.gray800, margin: "3px 0 2px", lineHeight: 1.3, wordBreak: "keep-all", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
  cardAuthor: { fontSize: 12, color: C.gray400, margin: 0 },
  arrow:      { flexShrink: 0, fontSize: 14, color: C.pink, fontWeight: 700 },
  overlay: { position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end", justifyContent: "center" },
  modal: { width: "100%", maxWidth: 480, background: C.white, borderRadius: "28px 28px 0 0", padding: "20px 24px 40px", position: "relative", maxHeight: "85vh", overflowY: "auto", display: "flex", flexDirection: "column", gap: 0 },
  handle: { width: 36, height: 4, borderRadius: 2, background: C.gray200, margin: "0 auto 16px" },
  closeBtn: { position: "absolute", top: 16, right: 16, width: 32, height: 32, borderRadius: "50%", border: "none", background: C.gray100, color: C.gray500, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Noto Sans KR', sans-serif" },
  modalTop: { display: "flex", gap: 16, alignItems: "flex-start", marginBottom: 16 },
  modalCover: { flexShrink: 0, width: 72, height: 96, borderRadius: 10, background: C.pinkLight, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", border: `1.5px solid ${C.pinkLight}` },
  modalCoverImg: { width: "100%", height: "100%", objectFit: "cover" },
  modalInfo: { flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 4, paddingTop: 4 },
  ...
  modalSummary: { fontSize: 14, lineHeight: 1.85, color: C.gray700, margin: "0 0 20px", wordBreak: "keep-all" },
  detailBtn: { width: "100%", padding: "14px 0", borderRadius: 16, border: "none", background: `linear-gradient(135deg, ${C.pink}, ${C.purple})`, color: C.white, fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "'Noto Sans KR', sans-serif", boxShadow: "0 8px 20px rgba(244,114,182,0.35)" },
  spinner: { width: 40, height: 40, borderRadius: "50%", margin: "0 auto", border: `3px solid ${C.pinkLight}`, borderTopColor: C.pink, animation: "spin-cw 1s linear infinite" },
  retryBtn: { padding: "12px 28px", borderRadius: 18, border: "none", cursor: "pointer", background: `linear-gradient(135deg, ${C.pink}, ${C.purple})`, color: C.white, fontSize: 14, fontWeight: 800, fontFamily: "'Noto Sans KR', sans-serif" },
};